...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    {
        char nameString[100] = "我是2代海龟机器人";
        text = nameString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.find("你的设备") != std::string::npos 
         || dataString.find("构成") != std::string::npos)
    {
        char eageString[100] = "我由深度相机,机械臂和底座组成";
        text = eageString;
        std::cout<<text<<std::endl;
    }
    ...
    {
        char helpString[100] = "我可以报出时间，也可以给你说说我看到了什么。我还能跳舞和挥手";
        text = helpString;
        std::cout<<text<<std::endl;
    }
	...
	else if(dataString.find("你看到") !=std::string::npos){
			return;
	}
	else if(dataString.find("跳舞")!=std::string::npos){
		char infoString[50]="好的，很高兴为你服务。";
		text=infoString;
	}
	else if(dataString.find("给我画")!= std::string::npos){
		char infoString[50]="我会努力尝试,但效果可能不理想。";
		text=infoString;
	}
	else if(dataString.find("手")!=std::string::npos){
		char infoString[50]="机械臂就绪。";
		text=infoString;
	}
    else
    {
...
int main(int argc, char* argv[])
{
	...
	//const char* login_params         = "appid = 11adbb4e, work_dir = ."
